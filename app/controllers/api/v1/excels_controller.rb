class Api::V1::ExcelsController < ApplicationController
  ...
  def excels_params
    params.require(:excel).permit(:user_id, :fees, data: [:Ticker, :NetProfit, :GrossProfit, :ShareCount, :TimeStamp, :Date, :Commissions, :SharesTraded, :Side])
  end

  def update_params
    params.permit(:user_id, :fees, data: [:Ticker, :NetProfit, :GrossProfit, :ShareCount, :TimeStamp, :Date, :Commissions, :SharesTraded, :Side])
  end
end